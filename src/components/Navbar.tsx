...
export default function Navbar() {
  ...
  return (
    <nav
      id="navbar"
      className="fixed top-4 left-0 right-0 z-50 transition-all duration-300 px-4 sm:px-6 lg:px-8 2xl:px-4"
    >
      <div 
        className={`mx-auto transition-all duration-300 rounded-full border max-w-7xl 2xl:max-w-[1580px] ${
          scrolled
            ? 'bg-white/90 backdrop-blur-xl shadow-[0_10px_30px_rgba(0,0,0,0.05)] border-slate-200/60 px-6 py-2.5'
            : 'bg-white/55 backdrop-blur-md border-slate-200/30 px-6 py-3'
        }`}
      >
        <div className="flex justify-between items-center">
          ...
          <a href="#" className="flex items-center gap-3.5 group min-w-0">
            ...
            <img
              ...
              className="h-14 sm:h-16 w-auto object-contain transition-transform duration-300 group-hover:scale-[1.03] shrink-0"
            />

            <div className="flex flex-col min-w-0">
              <div className="flex items-center gap-2.5">
                <span className="font-display font-extrabold text-lg sm:text-xl md:text-2xl leading-tight text-slate-950 tracking-tight group-hover:text-iamh-navy transition-colors whitespace-normal sm:whitespace-nowrap">
                  Indian Academy of Mental Health
                </span>
                <span className="hidden sm:inline-block px-2 py-0.5 text-[10px] font-mono font-bold tracking-wider bg-emerald-50 text-emerald-800 rounded border border-emerald-100/75 uppercase whitespace-nowrap">
                  Est. 2021
                </span>
              </div>
              <span className="hidden sm:block text-[11px] sm:text-xs font-mono text-slate-500 font-semibold tracking-wide whitespace-nowrap">
          ...
          {/* Desktop Navigation */}
          <div className="hidden 2xl:flex items-center gap-0.5 xl:gap-1.5">
            {navLinks.map((link) => (
              <a
                key={link.name}
                href={link.href}
          ...
          {/* Mobile menu button */}
          <div className="flex 2xl:hidden shrink-0">
            <button
      ...
      {/* Mobile Menu */}
      <div className={`2xl:hidden transition-all duration-300 ease-in-out overflow-hidden ${isOpen ? 'max-h-screen border-b border-slate-200/55 shadow-md mt-2 rounded-2xl mx-4' : 'max-h-0'}`}>
